...
class XtwSpider(scrapy.Spider):
    ...
    def parse_item(self, response, **kwargs):
        try:
            ...
            item['content'] = response.css('.texttit_m1').extract_first() if response.css(
                '.texttit_m1') else response.css('.article-box').extract_first()
            item['website'] = '信托网'
            item['link'] = kwargs['url']
            item['spider_name'] = 'xtw'
            item['txt'] = ''.join(response.css('.texttit_m1 *::text').extract()) if response.css(
                '.texttit_m1') else ''.join(response.css('.article-box *::text').extract())
