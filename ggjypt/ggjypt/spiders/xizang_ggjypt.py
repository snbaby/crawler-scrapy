...
class XizangGgjyptSpider(scrapy.Spider):
    ...
    def parse_pagenum(self, response):
        ...
        try:
            ...
            if not self.add_pagenum:
                return int(response.css('.pages-list li:nth-child(1) a::text').extract_first().split('/')[1].replace('页','').strip())
    ...
    def parse(self, response, **kwargs):
        print(3)
        for href in response.css('.article-list-old a').extract():
            try:
    ...
    def pares_item(self, response, **kwargs):
        ...
        try:
            ...
            item['time'] = response.css('.div-title2::text').extract_first().split('浏览')[0].replace('发布时间：','').strip()
            item['source'] = ''
