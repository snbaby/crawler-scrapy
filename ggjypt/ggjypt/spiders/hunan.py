...
class hunanSzfwjSpider(scrapy.Spider):
    name = 'hunan_ggjypt'
    custom_settings = {
        'SPIDER_MIDDLEWARES': {
    ...
    def start_requests(self):
        try:
            url = "http://www.hnsggzy.com/gczb/index.jhtml"
    ...
    def parse_item(self, response, **kwargs):
        if response.text:
            try:
                ...
                item['website'] = '湖南省公共资源交易服务平台'
                item['module_name'] = '湖南省-公共交易平台'
                item['spider_name'] = 'hunan_ggjypt'
                item['txt'] = "".join(response.xpath('//div[@class="div-article2"]//text()').extract())
                item['appendix_name'] = ";".join(response.xpath('//div[@class="div-article2"]//a[contains(@href,"pdf") or contains(@href,"doc") or contains(@href,"docx") or contains(@href,"xls")]/text()').extract())
                item['link'] = response.request.url
                item['appendix'] = ";".join(response.xpath('//div[@class="div-article2"]//a[contains(@href,"pdf") or contains(@href,"doc") or contains(@href,"docx") or contains(@href,"xls")]/@href').extract())
                print(
                    "===========================>crawled one item" +
