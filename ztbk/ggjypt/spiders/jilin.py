...
class TianJinSzfwjSpider(scrapy.Spider):
    ...
    def start_requests(self):
        try:
            url = "http://www.jlsggzyjy.gov.cn/jlsztb/jyxx/"
    ...
    def parse_type(self, response):
        for href in response.xpath('//li[@class="wb-tree-item"]/a/@href'):
            try:
    ...
    def parse_more(self, response,**kwargs):
        for selector in response.xpath('//div[@class="ewb-he200 ewb-mt10"]'):
            if selector.xpath('./div[@class="ewb-com-bd"]'):
                try:
    ...
    def parse_pagenum(self, response):
        try:
            ...
            if response.xpath('//a[@class="wb-page-default wb-page-number wb-page-family"]/text()'):
                return int(response.xpath('//a[@class="wb-page-default wb-page-number wb-page-family"]/text()').re(r'([1-9]\d*\.?\d*)')[1])
            else:
                return 0
        except Exception as e:
            logging.error(self.name + ": " + e.__str__())
    ...
    def parse(self, response):
        for selector in response.xpath('//li[@class="ewb-com-item clearfix"]'):
            try:
                item = {}
                item['title'] = selector.xpath('./div/a/text()').extract_first().strip()
    ...
    def parse_item(self, response,**kwargs):
        if response.text:
            try:
                ...
                item['website'] = '吉林省公共资源交易服务平台'
                item['module_name'] = '吉林省-公共交易平台'
                item['spider_name'] = 'jilin_ggjypt'
                item['txt'] = "".join(response.xpath('//div[@id="mainContent"]//text()').extract())
                item['appendix_name'] = ";".join(response.xpath('//div[@id="mainContent"]//a[contains(@href,"pdf") and contains(@href,"word") and contains(@href,"xls")]/text()').extract())
                item['link'] = response.request.url
                item['appendix'] = ";".join(response.xpath('//div[@id="mainContent"]//a[contains(@href,"pdf") and contains(@href,"word") and contains(@href,"xls")]/@href').extract())
                print(
                    "===========================>crawled one item" +
